  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <button
          onClick={async () => {
            if (!confirm('确定要清理所有过期信息吗？')) return;
            try {
              const res = await fetch('/api/admin/cleanup', { method: 'POST' });
              const data = await res.json();
              alert(data.message || '清理完成');
              if (data.cleaned > 0) fetchStats();
            } catch { alert('清理失败'); }
          }}
          className="ml-auto px-3 py-1.5 rounded-lg bg-orange-50 text-orange-600 border border-orange-300 text-xs font-bold hover:bg-orange-100 transition-colors"
        >
          🧹 清理过期
        </button>
      </div>
